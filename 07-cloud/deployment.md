...
Confirm a stable .NET SDK 10 or later with the [setup checks](../03-prerequisites/README.md#check-before-installing). Don't replace compatible runtimes merely to match a screenshot.
...
The template uses a **user-assigned managed identity**, an Azure identity created separately and then assigned to the app.
`AZURE_CLIENT_ID` identifies that identity. `KeyVaultName` selects the vault.
`InitializeDatabase` controls automatic schema creation. These setting names must match the supplied template.

You'll also need **design-time support** so EF Core tooling can inspect the model without starting the web app.
Here, the tooling generates SQL to create tables and seed books without connecting to a live database.

In Visual Studio, open `shared-legacy-app\BookCatalog.sln`.
In Copilot Chat, ask the modernization agent to execute only the approved application-preparation group:

```text
@Modernize Prepare my learner application for the reviewed Azure plan.
Use KeyVaultName and AZURE_CLIENT_ID for Key Vault access with the user-assigned managed identity.
Keep local startup independent of Azure when KeyVaultName is absent.
Allow automatic schema creation only when InitializeDatabase is true and the database is BookCatalogModernizedLab.
Keep saved edits across restarts. Disable automatic schema creation in Azure.
Add design-time support to generate schema SQL, including the demo seed books, without a live database connection.
Don't create Azure resources, deploy, or commit. Stop for diff review and local checks.
```
